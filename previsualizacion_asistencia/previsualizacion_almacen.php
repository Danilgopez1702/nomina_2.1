...
while ($data = mysqli_fetch_assoc($sql)){
    ...
    if($horario < $hora1){
        ...
        if($catch_error[1] == 0){ 
            ...
            if($falta_nocheco > 0){
                $sql_insert =  mysqli_query($con,"INSERT INTO `previsualizacion`(`id_empleado`, `status`, `fecha_previsualizacion`) 
                VALUES ('$emp', 2, '$hoy')");
            }
        }else{
            ...
            if($hora > $inicia_retardo  && $hora < $finaliza_retardo){
                ...
                $sql_select_check_retardo =  mysqli_query($con,"SELECT * FROM `asistencia` WHERE `id_empleado` = '$emp' && `status_asistencia` = 1 && `tipo_asistencia` = 3"); 
                ...
                if($no_check_retardo != 1){
                    ...
                    $sql_select_fretardo =  mysqli_query($con,"SELECT * FROM `asistencia` WHERE `id_empleado` = '$emp' && `status_asistencia` = 1 && `tipo_asistencia` = 1"); 
                    $no_retardo = mysqli_num_rows($sql_select_fretardo);
        
                    if($no_retardo > 2){
                        $sql_insert =  mysqli_query($con,"INSERT INTO `previsualizacion`(`id_empleado`, `status`, `fecha_previsualizacion`, `hora_previsualizacion`) 
                        VALUES ('$emp', 3, '$fecha', '$hora')");
        
                    }else{
                        $sql_insert =  mysqli_query($con,"INSERT INTO `previsualizacion`(`id_empleado`, `status`, `fecha_previsualizacion`, `hora_previsualizacion`) 
                        VALUES ('$emp', 1, '$fecha', '$hora')");
                    }
                }else {                 
                    $sql_insert =  mysqli_query($con,"INSERT INTO `previsualizacion`(`id_empleado`, `status`, `fecha_previsualizacion`, `hora_previsualizacion`, `status`, `comentario`) 
                    VALUES ('$emp', 1, '$fecha', '$hora')");
                }
...
$ayer_dia = date("N",strtotime("-1 days"));

$sql_1 =  mysqli_query($con,"SELECT * FROM `empleados` AS e INNER JOIN `departamento` AS d ON d.id_departamento = e.id_departamento WHERE `checador` = $oficina && `turno_departamento` = 2");

while ($data = mysqli_fetch_assoc($sql_1)){
    $q = 5;
    $emp = $data['id_empleado'];
    $empleado = substr($emp, 7);
    /************** SABER QUE HORARIO TIENE ESE DEPARTAMENTO  ***********************/
    ...
    if($q > 1){
        ...
        if($catch_error[1] == 0){ 
            ...
            //descansos sabado o domingo
                ...
                //revisar si no tiene vacaciones en este dia 
                if($data_descanso < 1){
                    $sql_select_vacacion =  mysqli_query($con,"SELECT * FROM `vacaciones` WHERE `fecha_vacacion` = '$ayer' && `id_empleado` = '$emp'");
                    $data_vacacion = mysqli_num_rows($sql_select_vacacion);
                    //no tiene vacaciones
                    //revisar guardia
                    if($data_vacacion < 1){
