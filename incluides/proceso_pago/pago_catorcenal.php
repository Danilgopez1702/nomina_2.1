<?php
  ...
  $sql_select = mysqli_query($conexion, "SELECT * FROM empleados WHERE status = 1 AND frecuencia_pago IN (7, 14)");
...
//Condicional por fechas 
        ...
        if($fe > $fe1){
          ...
          $fecha_uno = strtotime ( ' + 14 day', strtotime ($fe1));
...
//Redondear y dar estetica al formato de dinero 

      $t=number_format($total, 2, '.', ',' );

//Insertamos en la tabla de pago
            
      $sql = mysqli_query($conexion, "INSERT INTO historico (id_empleado, nombre, monto_pago,faltas,prestamo,hora_extra,diario,infonavit, modalidad_p, status ,fecha_historico , aguinaldo, monto_aguinaldo,monto_total) 
      VALUES ('$id','$nc','$total','$faltas_totales','$Prestado_total','$hrs','$sd','$infona', '$modal', '1', '$fecha_catorcenal', 1, 0, $total  )");
