...
$url = "10.2.3.10/ISAPI/AccessControl/AcsEvent?format=json";
$username = "admin";
$password = "CARLOS1985";
...
$sql =  mysqli_query($con,"SELECT * FROM `empleados` AS e INNER JOIN `departamento` AS d ON d.id_departamento = e.id_departamento WHERE `checador` = 1 ");
...
while ($data = mysqli_fetch_assoc($sql)){
  ...
  if ($ch != null) curl_close($ch);
    ...
    if($catch_error[1] == 0){ 
      ...
      if($falta_nocheco > 0){
        $sql_insert =  mysqli_query($con,"INSERT INTO `asistencia`(`id_empleado`, `tipo`, `fecha_asistencia`, `status`,	`comentario`) 
        VALUES ('$emp', 2, '$ayer', 1, ' NO CHECO ')");
      }
    }else{
      ...
      if($hora > $inicia_retardo  && $hora < $finaliza_retardo){
        ...
        if($no_check_retardo != 1){
          ...
          if($no_retardo > 2){
            ...
            $sql_insert =  mysqli_query($con,"INSERT INTO `asistencia`(`id_empleado`, `tipo`, `fecha_asistencia`, `hora_asistencia`, `status`, `comentario`) 
            VALUES ('$emp', 3, $fecha, $hora, 1, 'falta por retardos acumulados'])");
        ...
        $sql_insert =  mysqli_query($con,"INSERT INTO `asistencia`(`id_empleado`, `tipo`, `fecha_asistencia`, `hora_asistencia`, `status`, `comentario`) 
        VALUES ('$emp', 2, $fecha, $hora, 1, ' falta por sobrepasar los 15 min de tolerancia ')");
      }
    }
}
